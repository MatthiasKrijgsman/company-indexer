...
def _address_hint(address: Address | None) -> str:
    """Build a short location hint for the Serper query from the hoofdvestiging.

    City only: adding street/house/postcode starves recall because most
    company web pages mention the KVK number but not the full registered
    address. City is the broadest hint that still disambiguates.
    """
...
@router.post(
    ...
    response_model=WebsiteSearchDetail,
)
async def trigger_website_search(
    ...
    # Treat the first address as the hoofdvestiging until we model it explicitly.
    hint = _address_hint(company.addresses[0] if company.addresses else None)
    base = f'kvk "{kvk_number}" {hint}'.strip()
    ...
    return WebsiteSearchDetail.model_validate(record)
